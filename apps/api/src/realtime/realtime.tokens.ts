 ...
 * bus. `REALTIME_PRESENCE` is the concrete presence storage the tracker populates
 * and the presence endpoints read. Each resolves to `undefined` when
 * `PUBSUB_DRIVER` is `memory`, leaving the library on its single-instance defaults.
 */
...
/** Injection token for the concrete presence storage (or `undefined`). */
